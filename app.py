...
.stApp { background: var(--bg) !important; color: var(--white); font-family: 'DM Sans', sans-serif; }
#MainMenu, footer, header { visibility: hidden; }
.block-container { padding-top: 0; padding-bottom: 3rem; max-width: 1180px; }

/* ══ Force ALL Streamlit elements dark ══ */
...
.icon-btn-del > div.stButton > button:hover {
  ...
  box-shadow: 0 0 10px rgba(255,77,109,.25) !important;
  transform: none !important;
}

/* ══ Save / green button ══ */
.save-btn > div.stButton > button,
.save-btn > div.stButton > button:focus {
  background: #06200f !important;
  border: 1px solid #00e676 !important;
  color: #00e676 !important;
}
.save-btn > div.stButton > button:hover {
  background: rgba(0,230,118,.12) !important;
  box-shadow: 0 0 12px rgba(0,230,118,.2) !important;
}

/* ══ Cancel / red button ══ */
.cancel-btn > div.stButton > button,
.cancel-btn > div.stButton > button:focus {
  background: #200610 !important;
  border: 1px solid #ff4d6d !important;
  color: #ff4d6d !important;
}
.cancel-btn > div.stButton > button:hover {
  background: rgba(255,77,109,.12) !important;
  box-shadow: 0 0 12px rgba(255,77,109,.2) !important;
}

/* ══ Danger confirm button ══ */
.danger-btn > div.stButton > button,
.danger-btn > div.stButton > button:focus {
  background: #200610 !important;
  border: 1px solid #ff4d6d !important;
  color: #ff4d6d !important;
}
.danger-btn > div.stButton > button:hover {
  ...
  box-shadow: 0 0 12px rgba(255,77,109,.25) !important;
}
...
.save-btn > div.stFormSubmitButton > button:focus {
  background: #06200f !important;
  border: 1px solid #00e676 !important;
  color: #00e676 !important;
}
...
.cancel-btn > div.stFormSubmitButton > button:focus {
  background: #200610 !important;
  border: 1px solid #ff4d6d !important;
  color: #ff4d6d !important;
}
...
label { color: var(--muted) !important; font-size: .82rem !important; letter-spacing: .04em; }
...
div[data-baseweb="select"] div { color: var(--white) !important; }
...
.stat-card {
  background: var(--surface); border: 1px solid var(--border);
  border-radius: 14px; padding: 1.4rem 1rem; text-align: center; position: relative; overflow: hidden;
}
.stat-card::before {
  content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px;
  background: linear-gradient(90deg, transparent, var(--cyan), transparent);
...
/* ══ Pill tags ══ */
.pill { display: inline-block; border-radius: 20px; padding: .18rem .7rem; font-size: .7rem; font-weight: 600; letter-spacing: .04em; margin-right: .3rem; }
.pill-yes  { background: rgba(255,77,109,.12);  color: #ff4d6d; border: 1px solid rgba(255,77,109,.35); }
.pill-no   { background: rgba(0,230,118,.10);   color: #00e676; border: 1px solid rgba(0,230,118,.3);  }
.pill-info { background: rgba(0,229,255,.08);   color: #00e5ff; border: 1px solid rgba(0,229,255,.25); }

/* ══ Edit / delete panels ══ */
.edit-panel {
  background: #080f1a; border: 1px solid rgba(0,229,255,.22); border-top: none;
  border-radius: 0 0 10px 10px; padding: 1.2rem 1.4rem 1.4rem; margin-bottom: .4rem;
}
.del-panel {
  background: #100509; border: 1px solid rgba(255,77,109,.22); border-top: none;
  border-radius: 0 0 10px 10px; padding: 1rem 1.4rem; margin-bottom: .4rem;
}

/* ══ Scan detail card ══ */
.scan-card { background: #080f1a; border: 1px solid var(--border); border-radius: 12px; padding: 1.2rem 1.4rem; margin-bottom: .6rem; }
...
.scan-detail-label { font-size: .68rem; color: var(--muted); letter-spacing: .06em; text-transform: uppercase; }
.scan-detail-value { font-size: .88rem; color: var(--white); font-weight: 500; }
...
def build_model(num_classes):
    ...
    m = Sequential([Input(shape=(IMAGE_SIZE,IMAGE_SIZE,3)), base, Flatten(), Dropout(0.3),
                    Dense(128,activation='relu'), Dropout(0.2), Dense(num_classes,activation='softmax')])
...
def show_auth():
    ...
    with col:
        ...
        if mode == "login":
            ...
            new_n   = s2.text_input("Full Name",placeholder="Your full name")
            new_r   = st.selectbox("Role",["Radiologist","Doctor","Technician","Admin"])
            new_p   = st.text_input("Password",type="password",placeholder="Min 6 characters")
            conf_p  = st.text_input("Confirm Password",type="password",placeholder="Repeat password")
            if st.button("✨  Create Account", use_container_width=True):
                if not all([new_u,new_n,new_p,conf_p]):
                    ...
                    users[new_u]={"name":new_n,"role":new_r,"password":hash_pw(new_p),"created":datetime.now().isoformat()}
...
def show_app():
    ...
    t1,t2,t3,t4,t5 = st.tabs(["🏠  Home","🔍  Scan Analysis","👥  Patient Records","📋  Patient Reports","📊  Dashboard"])
    ...
    with t1:
        ...
        for col,icon,num,lbl in [(c1,"👥",total_p,"Total Patients"),(c2,"🔬",total_s,"Scans Processed"),
                                  (c3,"🔴",tumor_s,"Tumors Detected"),(c4,"✅",clear_s,"Clear Scans")]:
            col.markdown(f'<div class="stat-card"><div class="stat-icon">{icon}</div><div class="stat-num">{num}</div><div class="stat-label">{lbl}</div></div>', unsafe_allow_html=True)
        ...
        if recent:
            ...
            st.markdown('<div class="empty-state" style="min-height:130px"><span style="font-size:2rem;opacity:.2">🔬</span><p style="color:#6a8a9a;font-size:.88rem;margin:0">No scans yet.</p></div>', unsafe_allow_html=True)
                    ...
                    if st.button("💾  Save & Add Patient"):
                        if np_name:
                            ...
                            patients.append({"id":pid,"name":np_name,"age":np_age,"gender":np_gender,"contact":np_contact,
                                             "notes":"","dob":"","diabetic":"Unknown","blood_group":"Unknown",
                                             "blood_pressure":"Unknown","blood_sugar":"Unknown","cholesterol":"Unknown",
                                             "allergies":"","blood_report":"","registered":datetime.now().strftime("%Y-%m-%d"),"scans":[se["id"]]})
    ...
    with t3:
        ...
        with pr_left:
            ...
            filtered = ([p for p in patients if search.lower() in p["name"].lower() or
            ...
            if not filtered:
                st.markdown('<div class="empty-state" style="min-height:150px"><span style="font-size:2.2rem;opacity:.2">👥</span><p style="color:#6a8a9a;font-size:.88rem;margin:0">No patients found</p></div>', unsafe_allow_html=True)
            else:
                for p in filtered:
                    pid     = p.get("id","")
                    ...
                    if st.session_state.editing_patient == pid:
                        ...
                        st.markdown(f'<p style="font-family:Orbitron,sans-serif;font-size:.68rem;color:var(--cyan);letter-spacing:.1em;margin-bottom:.8rem">✏️ EDITING — {p["name"]}</p>', unsafe_allow_html=True)
                        with st.form(key=f"edit_form_{pid}"):
                            st.markdown('<p style="color:var(--muted);font-size:.78rem;margin:.2rem 0 .5rem">Basic Info</p>', unsafe_allow_html=True)
                            ...
                            st.markdown('<p style="color:var(--muted);font-size:.78rem;margin:.8rem 0 .5rem">Medical Info</p>', unsafe_allow_html=True)
                    ...
                    if st.session_state.confirm_del_pat == pid:
                        ...
                        st.warning(f"⚠️ Delete **{p['name']}**? All their scans will be permanently removed.")
        ...
        with pr_right:
            with st.expander("➕  Add New Patient",expanded=True):
                ...
                st.markdown('<p style="color:var(--cyan);font-size:.72rem;font-family:Orbitron,sans-serif;letter-spacing:.09em;margin:.9rem 0 .4rem">🩺 MEDICAL INFO</p>', unsafe_allow_html=True)
                b1,b2,b3    = st.columns(3)
                add_diab    = b1.selectbox("Diabetic",["Unknown","Yes","No"],key="add_diabetic")
                ...
                st.markdown('<p style="color:var(--cyan);font-size:.72rem;font-family:Orbitron,sans-serif;letter-spacing:.09em;margin:.9rem 0 .2rem">🧪 BLOOD TEST REPORT</p>', unsafe_allow_html=True)
                st.caption("Optional — patient can be added without uploading.")
                blood_file  = st.file_uploader("Upload Report (PDF/Image)",type=["pdf","jpg","jpeg","png"],key="blood_upload")
                ...
                if st.button("➕  Add Patient",use_container_width=True):
                    if add_name:
                        ...
                        blood_saved = ""
                        if blood_file:
                            rp = f"data/blood_reports/{pid}_{blood_file.name}"
                            with open(rp,"wb") as bf: bf.write(blood_file.read())
                            blood_saved = rp
                        patients.append({
                            "id":pid,"name":add_name,"age":int(add_age),"gender":add_gender,
                            "contact":add_contact,"dob":add_dob,"notes":add_notes,
                            "diabetic":add_diab,"blood_group":add_bg,"blood_pressure":add_bp,
                            "blood_sugar":add_bs,"cholesterol":add_chol,"allergies":add_allerg,
                            "blood_report":blood_saved,"registered":datetime.now().strftime("%Y-%m-%d"),"scans":[],
                        })
                        save_data("patients.json",patients)
    ...
    with t4:
        ...
        if not patients:
            ...
            if sel_pat:
                ...
                diab     = sel_pat.get("diabetic","Unknown")
                bg       = sel_pat.get("blood_group","Unknown")
                bp       = sel_pat.get("blood_pressure","Unknown")
                bs       = sel_pat.get("blood_sugar","Unknown")
                chol     = sel_pat.get("cholesterol","Unknown")
                allerg   = sel_pat.get("allergies","—")

                st.markdown("<br>", unsafe_allow_html=True)

                # Patient summary
                av_col,info_col = st.columns([0.07,1])
                with av_col:
                    st.markdown(f"""
                    <div style="width:50px;height:50px;border-radius:50%;margin-top:.3rem;
                         background:linear-gradient(135deg,#006680,#00e5ff);
                ...
                with info_col:
                    ...
                    <div style="padding:.15rem 0">
                      <div style="font-family:'Orbitron',sans-serif;font-size:.98rem;color:var(--white);font-weight:700;letter-spacing:.05em">{sel_pat['name']}</div>
                ...
                def med_tile(col,label,value,color="#e8f4f8"):
                    col.markdown(f"""
                    <div style="background:#080f1a;border:1px solid #1a2a40;border-radius:10px;
                         padding:.75rem .8rem;text-align:center">
                      <div style="font-size:.63rem;color:var(--muted);letter-spacing:.07em;text-transform:uppercase;margin-bottom:.3rem">{label}</div>
                      <div style="font-size:.88rem;font-weight:600;color:{color}">{value}</div>
                    </div>""", unsafe_allow_html=True)
                ...
                if allerg and allerg not in ["—",""]:
                    st.markdown(f'<div class="rbox r-amber" style="margin-top:.5rem">⚠️ <strong>Allergies:</strong> {allerg}</div>', unsafe_allow_html=True)
                if sel_pat.get("notes"):
                    st.markdown(f'<div class="rbox r-cyan" style="margin-top:.4rem">📝 <strong>Notes:</strong> {sel_pat["notes"]}</div>', unsafe_allow_html=True)
                br = sel_pat.get("blood_report","")
                if br and os.path.exists(br):
                    st.markdown(f'<div class="rbox r-cyan" style="margin-top:.4rem">🧪 <strong>Blood Report on file:</strong> {os.path.basename(br)}</div>', unsafe_allow_html=True)

                st.markdown("<br>", unsafe_allow_html=True)
                st.markdown(f'<div class="sec">🔬 Scan History <span style="font-size:.74rem;color:var(--muted);font-family:\'DM Sans\',sans-serif;font-weight:400;letter-spacing:0">&nbsp;{len(pt_scans)} scan(s)</span></div>', unsafe_allow_html=True)
                ...
                if not pt_scans:
                    ...
                    for i,s in enumerate(reversed(pt_scans)):
                        ...
                        d_col        = "#00e676" if diag=="notumor" else ("#ffab40" if conf<0.6 else "#ff4d6d")

                        with st.expander(f"🔬 Scan #{scan_num}  ·  {s.get('date','')}  ·  {diag.upper()}",expanded=(i==0)):
                            st.markdown(f"""
                            <div class="scan-card">
                              <div style="font-size:1rem;font-weight:700;color:{d_col};margin-bottom:.5rem">{result_label}</div>
                              <div style="background:rgba(0,229,255,.05);border-radius:7px;padding:.45rem .8rem;display:inline-block;font-size:.85rem;color:var(--white)">
                                AI Confidence: <strong style="color:{d_col}">{conf*100:.1f}%</strong>
                              </div>
                              <div class="scan-detail-grid">
                                ...
                                <div class="scan-detail-item">
                                  ...
                                  <span class="scan-detail-value" style="color:var(--cyan);font-family:'Orbitron',monospace;font-size:.76rem">{scan_id}</span>
                                </div>
                              </div>
                            </div>""", unsafe_allow_html=True)
                            ...
                            st.markdown("---")
                            st.markdown('<p style="color:var(--muted);font-size:.75rem;margin-bottom:.3rem">⚠️ Danger Zone</p>', unsafe_allow_html=True)
    ...
    with t5:
        ...
        for col,icon,num,lbl in [(sc1,"👥",total_p,"Total Patients"),(sc2,"🔬",total_s,"Scans Processed"),
        ...
        if not scans:
            ...
            for s in reversed(scans):
                df_rows.append({
                    "Scan ID":s.get("id",""),"Patient":s.get("patient_name",""),

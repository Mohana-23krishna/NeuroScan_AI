...
div.stButton > button:hover { transform: translateY(-1px); box-shadow: 0 6px 22px rgba(0,229,255,.28); }

.icon-btn > div.stButton > button {
  background: rgba(0,229,255,.10) !important; border: 1px solid var(--cyan) !important;
  color: var(--cyan) !important; font-size: 1rem !important; padding: .28rem .55rem !important;
  border-radius: 7px !important; letter-spacing: 0 !important; min-width: unset !important;
  width: auto !important; text-transform: none !important; transition: all .15s !important;
}
.icon-btn > div.stButton > button:hover {
  background: rgba(0,229,255,.25) !important; box-shadow: 0 0 10px rgba(0,229,255,.35) !important; transform: none !important;
}

.icon-btn-del > div.stButton > button {
  background: rgba(255,77,109,.10) !important; border: 1px solid var(--red) !important;
  color: var(--red) !important; font-size: 1rem !important; padding: .28rem .55rem !important;
  border-radius: 7px !important; letter-spacing: 0 !important; min-width: unset !important;
  width: auto !important; text-transform: none !important; transition: all .15s !important;
}
.icon-btn-del > div.stButton > button:hover {
  background: rgba(255,77,109,.25) !important; box-shadow: 0 0 10px rgba(255,77,109,.35) !important; transform: none !important;
}

.save-btn > div.stButton > button { background: linear-gradient(135deg, #00703a, #00e676) !important; color: #000 !important; font-weight: 800 !important; }
.cancel-btn > div.stButton > button { background: linear-gradient(135deg, #7a0020, #ff4d6d) !important; color: #fff !important; }
.danger-btn > div.stButton > button { background: linear-gradient(135deg, #7a0020, #ff4d6d) !important; color: #fff !important; }

[data-testid="stFileUploader"] { background: var(--surface); border: 1px dashed var(--cdim); border-radius: 12px; padding: 1rem; }
...
.scan-card {
  ...
  border-radius: 12px; padding: 1.2rem 1.4rem; margin-bottom: .6rem;
}
.scan-card-header {
...
.pill {
  ...
  font-size: .72rem; font-weight: 600; letter-spacing: .05em; margin-right: .3rem;
}
...
def load_saved_model(path):
    ...
    try:
        ...
        try:
            model = load_model(path, compile=False); return model
        finally:
            kl.InputLayer.from_config = original_from_config
    except Exception:
        pass
    try:
        model = build_model(len(CLASS_LABELS)); model.load_weights(path); return model
...
def delete_scan(scan_id, scans, patients):
    ...
    for p in patients:
        if "scans" in p and scan_id in p["scans"]:
            p["scans"].remove(scan_id)
...
def show_app():
                ...
                if not os.path.exists(model_path):
                    ...
                    try:
                        ...
                        st.session_state.last_scan = {
                            "diagnosis":label,"confidence":conf,"all_probs":preds[0].tolist(),
                    ...
                    if st.button("💾  Save & Add Patient"):
                        if np_name:
                            ...
                            patients.append({"id":pid,"name":np_name,"age":np_age,"gender":np_gender,"contact":np_contact,
                                             ...
                                             "blood_sugar":"Unknown","cholesterol":"Unknown","allergies":"","scans":[se["id"]]})
    ...
    with t3:
                ...
                for p in filtered:
                        ...
                        <div class="patient-row">
                          ...
                          <div style="flex:1;min-width:0">
                            ...
                            <div style="margin-top:.35rem">
                              <span class="pill {'pill-yes' if diab=='Yes' else 'pill-no' if diab=='No' else 'pill-info'}">
                                💉 Diabetic: {diab}
                              </span>
                    ...
                    if st.session_state.editing_patient == pid:
                        ...
                        with st.form(key=f"edit_form_{pid}"):
                            ...
                            diab_opts  = ["Unknown","Yes","No"]; cur_diab = p.get("diabetic","Unknown")
                            e_diabetic = m1.selectbox("Diabetic", diab_opts, index=diab_opts.index(cur_diab) if cur_diab in diab_opts else 0)
                            ...
                            e_allerg = st.text_input("Allergies", value=p.get("allergies",""), placeholder="e.g. Penicillin, Pollen…")
                            ...
                            if submitted:
                                if not e_name.strip():
                                    ...
                                    p.update({"name":e_name.strip(),"age":int(e_age),"gender":e_gender,
                                              "contact":e_contact,"dob":e_dob,"diabetic":e_diabetic,
                                              "blood_group":e_bg,"blood_pressure":e_bp,"blood_sugar":e_bs,
                                              "cholesterol":e_chol,"allergies":e_allerg,"notes":e_notes})
                                    save_data("patients.json",patients); save_data("scans.json",scans)
        ...
        with pr_right:
            with st.expander("➕ Add New Patient", expanded=True):
                ...
                add_notes   = st.text_area("Medical History / Notes (optional)", key="add_notes", placeholder="Relevant history, prior conditions…")
                ...
                if st.button("➕  Add Patient"):
                    if add_name:
                        ...
                        if blood_file:
                            ...
                            with open(report_path, "wb") as bf:
                                bf.write(blood_file.read())
                        ...
                        patients.append({
                            ...
                            "diabetic": add_diabetic, "blood_group": add_bg,
                            "blood_pressure": add_bp, "blood_sugar": add_bs,
                            "cholesterol": add_chol, "allergies": add_allerg,
                            "blood_report": blood_report_saved,
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  TAB 4 — PATIENT REPORTS  (neat card layout)
    # ══════════════════════════════════════════════════════════════════════
    with t4:
                ...
                with st.container():
                    ...
                    with col_info:
                        ...
                        <div style="padding:.2rem 0">
                          <div style="font-family:'Orbitron',sans-serif;font-size:1rem;color:var(--white);font-weight:700;letter-spacing:.05em">
                            {sel_pat['name']}
                          </div>
                    ...
                    for i, s in enumerate(reversed(pt_scans)):
                        ...
                        with st.expander(f"🔬 Scan #{scan_num}  ·  {s.get('date','')}  ·  {diag.upper()}", expanded=(i==0)):
                            ...
                            <div class="scan-card">
                              ...
                              <div style="font-size:1.05rem;font-weight:700;color:{diag_color};margin-bottom:.6rem">
                                {result_label}
                              </div>
                              <div style="background:rgba(0,229,255,.05);border-radius:8px;padding:.5rem .8rem;
                                   display:inline-block;font-size:.88rem;color:var(--white)">
                            ...
                            with st.form(key=f"note_{scan_id}"):
                                new_note = st.text_area("Update / Add Doctor Notes", value=s.get("doctor_notes",""), placeholder="Clinical findings, recommendations…")
    ...
    with t5:
        ...
        if not scans:
            ...
            with da1:
                ...
                _,_,ats = ax.pie(list(dc.values()),labels=list(dc.keys()),
                                  colors=[pal.get(l,'#6a8a9a') for l in dc.keys()],
            ...
            with da2:
                ...
                if len(sd)>1:
                    ...
                    ax.bar(sd,[date_c[d] for d in sd],color='#00e5ff',width=0.4)
